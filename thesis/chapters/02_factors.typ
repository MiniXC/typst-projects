...
More recently, a new class of learned or neural audio codecs has emerged, which use deep learning to achieve unprecedented compression. These models, such as EnCodec @defossez_encodec_2023 and DAC @kumar_dac_2023, typically employ an autoencoder architecture. An encoder network applies a transformation $cal(R)$ to the raw waveform $S$ to create a compact, quantised latent representation $cal(R)(S)$, and a decoder network $f_theta$ reconstructs the audio from this representation to produce a synthetic signal $Syn = f_theta (cal(R)(S))$. By training end-to-end to minimise the reconstruction error between $S$ and $Syn$, these models learn to preserve the most perceptually salient information, achieving quality comparable or superior to traditional codecs at significantly lower bitrates. For example, EnCodec uses residual vector quantisation to create a hierarchical representation that can be streamed in real-time @defossez_encodec_2023, while DAC demonstrates high-fidelity reconstruction for general audio at bitrates as low as 1.5 kbps @kumar_dac_2023.

The discrete tokens $cal(R)(S)$ produced by these neural codecs constitute a powerful, reconstructible representation of the speech signal. Unlike self-supervised representations discussed in @02_ssl, which are optimised for downstream discriminative tasks, the goal of a codec's representation is faithful reconstruction. This property has made neural codec tokens a popular target for modern generative models. Instead of predicting complex spectrograms or waveforms directly, many state-of-the-art text-to-speech ($T arrow Syn$) and voice conversion ($S arrow Syn$) systems now generate these discrete audio tokens, which are then converted to a waveform $Syn$ using a pre-trained neural codec decoder $f_theta$ @wang_valle_2023@borsos_soundstorm_2023. This approach simplifies the generation task and has led to significant advances in the quality and controllability of synthetic speech. It can be applied to audio in general or speech alone @wu_codecsuperb_2024.

=== Self-supervised learning representations <02_ssl>

A prolific field of study when it comes to speech representations is #abbr.a[SSL]. In this a model is trained to predict pseudo-labels created from the input signal $S$ itself. The first of these models were introduced for #abbr.a[NLP] in which a percentage of tokens in the original data are masked and said models are trained to predict the masked tokens using cross entropy loss @devlin_bert_2019.

This methodology was adapted to speech by later works @schneider_wav2vec_2019@baevski_wav2vec_2020@hsu_hubert_2021@chen_wavlm_2022. These approaches typically process an input signal $S$ into 20-millisecond chunks which are passed through a #abbr.a[CNN] and discretised. HuBERT @hsu_hubert_2021 and WavLM @chen_wavlm_2022 use iterative clustering, while wav2vec and wav2vec 2.0 use vector quantisation, with discretisation using Gumbel softmax in the latter. Due to this, the latter family of models relies on contrastive loss while the former can use categorical cross entropy as in BERT @devlin_bert_2019. The resulting representations $cal(R)(S)$ achieve state-of-the-art results on several benchmarks and challenges @evain_lebenchmark_2021@yang_superb_2021@shi_mlsuperb_2023@tsai_superb-sg_2022@wang_fine-tuned_2021 -- they have also been expanded to cover many languages beyond English @conneau_xlsr_2021@boito_mhubert-147_2024. However useful these models and their representations are for downstream tasks, it is not clear in which ways the latent spaces learned by these models correlate with human perception. For example, #citep(<millet_humanlike_2022>) find that #abbr.pla[MFCC] predict systematic effects of contrasts between specific pairs of phones better than self-supervised models. Similar effects can be observed with tasks such as word segmentation @pasad_words_2024, and different layers of the model have been shown to correlate with different perceptual units @pasad_layer-wise_2021. It therefore stands to reason that while these reductive transformations are useful for a wide range of tasks, they do not intuitively correlate with human perception. In the following sections, we discuss more specific representations which more transparently correlate with different aspects of human perception.

=== Perceptually-grounded representations <02_perceptual>

In this work, we divide reductive transformations of a speech signal $S$ into three perceptually-grounded categories, which can be used for conditioning, i.e., as part of a set $cal(C)$.
