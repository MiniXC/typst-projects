...
The primary goal of Text-to-Speech (TTS) is to automatically convert a sequence of text into a natural-sounding utterance. #minorchange[Wording]["semantic" #sym.arrow.r "content"][This chapter provides a comprehensive overview of the systems designed to solve this complex "one-to-many" problem, where a single text or other #smallcaps[Content] input can correspond to countless valid acoustic realisations depending on factors like #smallcaps[Speaker] and #smallcaps[Prosody] introduced in @02_factors[Chapter].] This chapter will illustrate how TTS systems are fundamentally designed to generate and manipulate representations related to these factors, while using yet more representations of speech for conditioning and internal representations.
...
#minorchange[Wording]["semantic" #sym.arrow "content", #strike["raw"]][Beyond text-conditioned synthesis, these models produce speech without any explicit or implicit input, sampling instead from a learned distribution of speech sounds. Thus, these models aim to learn the underlying probability distribution $Q(S)$ directly. Early neural approaches, such as the original WaveNet architecture, demonstrated this by using an autoregressive model to predict low-level #smallcaps[Generic] audio samples one at a time, conditioned only on previous samples @oord_wavenet_2016. Textless models are foundational for tasks like audio completion or infilling but they require careful handling of sequence length and long-term coherence to avoid devolving into unstructured noise @tan_survey_2021. Recently, this form of generation has seen increased interest due to #abbr.pla[SLM] @lakhotia_gslm_2021 where a #abbr.a[LLM]-like system is trained to predict high-level acoustic units, usually contextualised speech embeddings -- however, even the latest systems still lack lexical coherence, with the text extracted from speech produced by these models achieving $<20%$ "win rate" against real text @park_speechssm_2025. While they usually do not perform as well as TTS, such approaches can be necessary when there is no script for the target language @chen_hokkien_2023.

==== Voice Conversion

While TTS maps a #smallcaps[Content] representation ($T$) to a #smallcaps[Generic] acoustic one ($S$), the related task of #emph[#abbr.a[VC]] transforms an input speech signal from a source style to a target style while preserving the linguistic content. VC conditions on a source utterance $S^"SRC"$ rather than text $T$:

$
tilde(S) tilde Q_theta (S | S^"SRC", Z)
$

Here, the conditioning set $Z$ typically contains a high-level #smallcaps[Speaker] embedding representing the target voice. A key challenge in VC is to effectively disentangle the #smallcaps[Content] content of the speech from the #smallcaps[Speaker] and #smallcaps[Prosody] factors, often using specialised feature extractors or autoencoder-based architectures @sisman_vc_2020. VC enables applications like real-time voice modification and dubbing, where the goal is to change the voice characteristics without relying on a transcript.]
...
For this thesis, we focus on TTS due to the abundance of openly available TTS models compared to #abbr.pla[VC] or #abbr.pla[SLM] as well as due to their controllability -- being able to condition on separate #smallcaps[Content], #smallcaps[Speaker] and optionally #smallcaps[Prosody] representations allows us to investigate the synthetic speech in more controlled experimental setups. Systems which condition on at least the former two are understood as multi-speaker, voice-cloning TTS systems, which make up the majority of modern TTS systems @tan_survey_2021. The #smallcaps[Content] representation is text or derived from text, such as contextualised text embeddings or phones. The #smallcaps[Speaker] representation is most commonly a high-level learned speaker embedding (see @02_speaker), but can also be another representation containing salient speaker information, such as a mid-level mel spectrogram of a reference utterance @eskimez_e2_2024 or even a high-level text prompt describing their identifying characteristics @lyth_parler_2024.

Unless stated otherwise, we treat voice cloning as the primary motivating use-case, and when we discuss voice cloning we primarily mean the #emph[zero-shot] setting (previously unseen speakers specified only by a reference utterance at inference time). Some of our controlled TTS-for-ASR experiments (Chapters 5–7) operate with speakers seen during TTS training, but still use explicit speaker representations for conditioning and control.
...
#minorchange[Wording]["semantic" #sym.arrow "content"][*Grapheme-to-Phoneme (G2P) Conversion:* This is often the final step, converting the normalised text into a mid-level #smallcaps[Content] representation: the phone sequence. This mapping from orthography to a phonetic representation like /s p iː tʃ/ is essential for accurate pronunciation, particularly in languages with irregular orthography like English or with polyphones in Chinese @yao_g2p_2015.]
...
#minorchange[Wording][#strike["raw"]][The legacy era is defined by modular systems. *Concatenative synthesis*, exemplified by the Festival toolkit @taylor_festival_1998, uses a frontend with full linguistic analysis to select waveform chunks from a database. As it concatenates existing audio, it does not require a vocoder. In parallel, *Statistical Parametric Speech Synthesis* (SPSS), seen in HTS @tokuda_hts_2013 and later the DNN-based Merlin toolkit @wu_merlin_2016, also uses a complex linguistic frontend but generates parameters for a Source-Filter vocoder like WORLD @morise_world_2016.]

The first wave of end-to-end neural models simplifies the frontend to basic tokenization and focuses on generating spectrograms. Tacotron @wang_tacotron_2017 is a autoregressive model that used the algorithmic Griffin-Lim @griffin_griffinlim_1984 method as its vocoder. A major breakthrough in quality was achieved with Tacotron 2 @shen_natural_2018, which pairs a similar spectrogram-prediction model with a powerful, though slow, Simple Autoregressive neural vocoder based on WaveNet @oord_wavenet_2016.

#minorchange[Wording][#strike["raw"], "Mel" #sym.arrow "mel"][The subsequent drive for efficiency leads to a family of non-autoregressive models that generate mel spectrograms in parallel. These systems, including FastSpeech, FastSpeech2, and GlowTTS, offload the final waveform generation to a separately trained, fast neural vocoder. As @fig:fig_tts_timeline shows, these models can be paired with various mel spectrogram based vocoders, with GAN-based models like HiFi-GAN @kong_hifigan_2020 being the most common choice due to their speed and quality. Diffusion-based and Flow-based vocoders are also viable options for this architecture.

Recent developments have focused on integrated end-to-end approaches and output representations. VITS @kim_vits_2021 represents a true end-to-end approach, jointly learning to align text and generate a waveform directly within a single model, thus having its own integrated vocoder. Concurrently, diffusion models have been applied to the main TTS task in systems like NaturalSpeech2 @tan_naturalspeech_2024 and StyleTTS2 @li_styletts_2023, which typically generate a high-quality mel spectrogram that is then rendered by a powerful GAN-based vocoder. The latest paradigm shift, represented by VALL-E @wang_valle_2023 and ParlerTTS @lyth_parler_2024, abandons spectrograms entirely.] These systems use a simplified frontend, sometimes leveraging an SSL Phoneme Model, Phoneme-level BERT @li_plbert_2023, for robust phonetic representations, and their acoustic model predicts a sequence of discrete tokens. The final waveform is then synthesised using a Neural Audio Codec decoder as the vocoder.

=== Architectures and Training Objectives

#minorchange[Wording]["semantic" #sym.arrow "content", "Mel" #sym.arrow "mel"][The development of modern TTS has been driven by the exploration of diverse neural network architectures and the training objectives used to optimise them. While legacy systems were modular by necessity, current state-of-the-art approaches are predominantly #abbr.pla[DNN]. These systems typically take as input a #smallcaps[Content] representation in the form of a character or phoneme sequence, and are trained to produce a #smallcaps[Generic] acoustic representation as output, most commonly a mel spectrogram or a sequence of discrete audio codec tokens. This section details the primary architectural paradigms that define how these models generate speech, and the training objectives used to guide their learning process.]
...
*#abbr.a[AR] Models:* These models generate the output sequence one step at a time, conditioning each step on the previously generated outputs. If we let the input linguistic sequence be $T$ and the target acoustic sequence be $S = (s_1, ..., s_K)$, an AR model factorises the probability sequentially:
...
$
The attention module calculates a context vector $c_k$ as the weighted sum of the encoder outputs, $c_k = sum_j alpha_(k,j) h_j$, which is then used to predict the output frame $s_k$. This process allows the model to dynamically learn the alignment between the text and the much longer audio sequence. While this approach produces high-quality speech, its sequential nature makes inference slow and can lead to error propagation. More recently, this paradigm has been applied to discrete token generation in models like VALL-E @wang_valle_2023, which use a Transformer architecture to autoregressively predict neural codec tokens.

*#abbr.a[NAR] Models:* To overcome the slow inference of AR models, NAR models were developed to generate the entire output sequence in parallel. These models assume conditional independence between the output frames given the full input conditioning:
...
Here we discuss the most common subjective listening test methodologies and best practices. Subjective tests are the gold standard for synthetic speech evaluation, however, there are drawbacks and trade-offs to any listening test since human behaviour can never be fully anticipated, especially across differing groups of listeners, spans of time, and sets of #abbr.a[TTS] systems. These tests aim to quantify perceptual qualities such as overall naturalness or speaker similarity, but their results are inherently variable due to listener biases and contextual factors @zielinski_bias_2008. #majorchange[Clarification][added clarification on near-parity/parity claims][Recent advancements in #abbr.a[TTS] have produced systems whose reported #abbr.a[MOS]/#abbr.a[CMOS] are sometimes not statistically distinguishable from ground truth under a specific test design and rater pool @chen_vall-e_2024. However, such near-parity claims should be interpreted cautiously: they are highly dependent on the evaluation protocol (e.g., ceiling effects, anchoring, and listener expertise) and do not necessarily imply parity in all perceptual dimensions, robustness across domains, or suitability for downstream tasks.]
...
[
#minorchange[Wording][Add reference to MUSHRA][*What is "range" of the data?* Does the test include ground truth (real) reference samples, and if so, are they explicitly labeled? The inclusion of ground truth clips enables direct comparison against real speech. Some protocols also include #emph[anchors]—intentionally degraded samples to calibrate the lower end of the scale—which is particularly characteristic of multi-stimulus designs like #abbr.a[MUSHRA] @schoeffler_mushra_2015. While anchors can improve rating consistency, they can also compress the upper range and make discrimination between top systems difficult @le_voting_2025.]
],
[
...
In practice, #abbr.a[MOS] involves presenting listeners with isolated audio samples and asking them to rate on the 5-point scale, with the labels being "Bad" (1) "Poor" (2), "Fair" (3), "Good" (4) and "Excellent" (5) @ITU_P800. Scores are then averaged across listeners to yield a system-level #abbr.a[MOS]. This method is widely used in #abbr.a[TTS] challenges, such as the Blizzard Challenge @king_blizzard_2008, where it has been employed to compare systems over time. However, criticisms include its susceptibility to ceiling effects in high-quality #abbr.a[TTS] systems, where scores cluster near 5, reducing discriminative power. Moreover, #abbr.a[MOS] ratings can shift over time as listener expectations evolve; for instance, systems rated highly in 2008 may score lower today due to advancements in #abbr.a[TTS] @le_maguer_back_2022. Despite these issues, #abbr.a[MOS] remains a benchmark for validating objective metrics.

#majorchange[Additional Context][contrast MOS to MUSHRA][
It is also important to distinguish #abbr.a[MOS] from #abbr.a[MUSHRA] (Multiple Stimuli with Hidden Reference and Anchor). In canonical #abbr.a[MOS], each trial rates a single stimulus in isolation. #abbr.a[MUSHRA], in contrast, is a multi-stimulus protocol in which listeners rate several systems for the same utterance on a continuous scale (often 0–100), typically with a known reference and at least one anchor @schoeffler_mushra_2015. In the #abbr.a[TTS] literature, the boundary between these paradigms is sometimes blurred in practice (e.g., presenting multiple samples for the same text on one page while still collecting 1–5 ratings), which can affect comparability across studies.]

===== Comparison MOS <03_cmos>

#majorchange[Definition][added A/B test definition][
Comparison MOS is an A/B-test-inspired variation of #abbr.a[MOS], in which two samples are presented and the listener is asked to compare and rate on a 7-point scale from -3 (A is much better than B) to +3 (A is much worse than B), with 0 indicating they are equivalent in naturalness (or quality). Here, an #emph[A/B test] refers to a pairwise evaluation where participants are presented with two alternatives and asked to express a preference or relative judgment; in audio, this typically requires randomising presentation order (to avoid systematic bias) and keeping lexical content matched. However, the same standardisation problem is present. E.g., sometimes a 13-point scale (from -6 to +6) is employed @li_styletts_2023 and even the name of the test is sometimes referred to as comparative instead of comparison #abbr.a[MOS].]
...
Typically evaluated on a 5-point scale from 1 (definitely different speakers) to 5 (definitely the same speaker), #abbr.a[SMOS] is essential for voice-cloning #abbr.a[TTS] systems @casanova_xtts_2024. #majorchange[Clarification][clarified zero-shot TTS use case][It is most commonly used in speaker-conditioned settings, including the #emph[zero-shot] use-case where a previously unseen speaker is specified only via a reference utterance at inference time. Throughout this thesis, we frequently discuss this speaker-conditioned / voice-cloning paradigm because it motivates speaker similarity evaluation.]
