...
*Diffusion Loss:* The training of diffusion models relies on a specialized objective. The model, $epsilon_theta$, is optimized to predict the noise $epsilon$ that was added to a clean sample $S_0$ to create a noised version $S_h$ at timestep#footnote[not to be confusied with the previous timesteps in a sequence used for AR model training] $h$. The simplified loss is typically an MSE loss at each timestep $h$:
$ cal(L)_(text("DDPM"))(theta) = EE_(S_0, bold(epsilon), h) [|bold(epsilon) - f (bold(S)_(h), h;theta)\|_2^2] $
By learning to predict the noise, the model effectively learns the score function of the data distribution. This allows it to generate high-fidelity samples that do not suffer from the oversmoothing issue of direct MSE prediction on the data itself. This is the core objective for diffusion-based TTS models like NaturalSpeech 2. Some state-of-the-art systems, such as StyleTTS 2 @li_styletts_2024, use a combination of these losses—for instance, a reconstruction loss (like L1), an adversarial loss, and perceptual feature matching losses—to achieve the best results.
