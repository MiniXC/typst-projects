...
The primary goal of Text-to-Speech (TTS) is to automatically convert a sequence of text into a natural-sounding utterance. This chapter provides a comprehensive overview of the systems designed to solve this complex "one-to-many" problem, where a single text or other #smallcaps[Content] input can correspond to countless valid acoustic realisations depending on factors like #smallcaps[Speaker] and #smallcaps[Prosody] introduced in @02_factors[Chapter]. This chapter will illustrate how TTS systems are fundamentally designed to generate and manipulate representations related to these factors, while using yet more representations of speech for conditioning and internal representations.
...
This makes the frontend, or text analysis module, a critical component of many TTS systems. Its purpose is to transform raw, unstructured text into a clean, structured linguistic feature representation suitable for the acoustic model @taylor_tts_2009. This process is fundamentally about converting the highest-level #smallcaps[Content] representations into a format that can be more easily mapped to acoustics. This can involve the following:
...
*Grapheme-to-Phoneme (G2P) Conversion:* This is often the final step, converting the normalised text into a mid-level #smallcaps[Content] representation: the phone sequence. This mapping from orthography to a phonetic representation like /s p iː tʃ/ is essential for accurate pronunciation, particularly in languages with irregular orthography like English or with polyphones in Chinese @yao_g2p_2015.
...
It is also important to distinguish #abbr.a[MOS] from #abbr.a[MUSHRA] (Multiple Stimuli with Hidden Reference and Anchor). In canonical #abbr.a[MOS], each trial rates a single stimulus in isolation. #abbr.a[MUSHRA], in contrast, is a multi-stimulus protocol in which listeners rate several systems for the same utterance on a continuous scale (often 0–100), typically with a known reference and at least one anchor @schoeffler_mushra_2015. In the #abbr.a[TTS] literature, the boundary between these paradigms is sometimes blurred in practice (e.g., presenting multiple samples for the same text on one page while still collecting 1–5 ratings), which can affect comparability across studies and contribute to ceiling effects.
...
Typically evaluated on a 5-point scale from 1 (definitely different speakers) to 5 (definitely the same speaker), #abbr.a[SMOS] is essential for voice-cloning #abbr.a[TTS] systems @casanova_xtts_2024. It is most commonly used in speaker-conditioned settings, including the #emph[zero-shot] use-case where a previously unseen speaker is specified only via a reference utterance at inference time. Throughout this thesis, we frequently discuss this speaker-conditioned / voice-cloning paradigm because it motivates speaker similarity evaluation. However, it is important to note that several of our controlled TTS-for-ASR experiments (Chapters 5–7) operate in a multi-speaker setting with speakers seen during TTS training, even though we still condition on explicit speaker representations (e.g., d-vectors) for control and consistency.
