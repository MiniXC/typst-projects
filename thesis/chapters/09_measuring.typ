...
There is no single ground truth for synthetic speech generation due to the one-to-many nature of the task. Instead, we frame TTS evaluation as a distributional similarity task. Let $D$ be an audio dataset and $X$ a feature extracted from it. We aim to quantify how well synthetic speech mirrors real speech by obtaining correlates of each factor and measuring their distance from both real speech datasets and noise datasets. TTSDS is the average of the resulting scores along the following perceptually motivated factors: (i) #smallcaps[Generic]: Overall distributional similarity, via SSL embeddings. (ii) #smallcaps[Speaker]: Realism of speaker identity. (iii) #smallcaps[Prosody]: Pitch, duration, and rhythm quality. (iv) #smallcaps[Intelligibility]: Recognition of the speech, using ASR-derived features.

#comic.comic((80mm, 40mm), "Comic table showing TTSDS factors (Generic, Speaker, Prosody, Intelligibility) and their associated features (e.g., SSL activations, speaker embeddings, pitch and rate metrics, ASR activations)", green) <ttsds2_features>
...
To allow us to average across features and factors, we derive a score, ranging from 0 (identical with a noise distribution) to 100 (identical with the real reference) for any synthetic speech feature distribution $P_hat(X|D_"syn")$ – the lexical content of the reference and synthetic speech do not need to match. We first compute the 2-Wasserstein distance of $P_hat(X|D_"syn")$ to each $P_hat(X|D_"noise")$ for a set of “distractor” noise datasets $D_"noise"$ and let the minimum of said values be $W_"noise"|(X)$. The 2-Wasserstein distance to the real speech distribution $P_hat(X|D_"real")$ is in turn $W_"real"(X)$ – using these terms, the normalized similarity score for a feature $X$ is simply defined as follows:
$ S(X) = 100 times W_"noise"(X) / (W_"real"(X) + W_"noise"(X)) $

#comic.comic((80mm, 40mm), "Comic density plot of F0 distributions for ground-truth, synthetic, and noise, with annotations for Wasserstein distances and score computation", blue) <fig_f0>
...
Updated factors and features. In this work, we modify the original TTSDS @minixhofer_ttsds_2024 to make it robust to several domains for each of its factors. INTELLIGIBILITY in TTSDS relies on Word Error Rate (WER). In preliminary experiments, these WER features resulted in low scores for real data across domains – to increase robustness, we use speech recognition models’ final-layer activations instead. For PROSODY, TTSDS originally used (a) the WORLD pitch contour @morise_world_2016, (b) masked-prosody-model embeddings @wallbridge_mpm_2025, and (c) token lengths (in frames) extracted from HuBERT @hsu_hubert_2021. We found that the token-length features lead to low scores for real speech. We instead compute the utterance-level speaking rate by dividing the number of deduplicated HuBERT tokens in an utterance by the number of frames. We do the same for the multilingual phone recogniser Allosaurus @li_allosaurus_2020, also included in the original. GENERIC uses the same HuBERT @hsu_hubert_2021 and wav2vec 2.0 @baevski_wav2vec_2020 features as in the original, but we also add WavLM @chen_wavlm_2022 features for increased diversity. The factors and their features are shown in the above figure. For multilingual use, we replace HuBERT with mHuBERT-147 @boito_mhubert-147_2024, and wav2vec 2.0 with its XLSR-53 counterpart @conneau_xlsr_2021.

=== English TTSDS: Correlations with listening tests across datasets

We now outline how we validate TTSDS to correlate with human scores across a variety of datasets.

#strong[Datasets from read speech to children’s speech]
...
WILD is created by scraping recent YouTube videos and extracting utterances, which tests the metrics’ ability to generalize to diverse speaking styles and recording conditions. Its data collection and processing are inspired by Emilia @he_emilia_2024. We scrape 500 English-language YouTube videos uploaded in 2025 using 10 different search terms which emphasise scripted and conversational speech alike. We perform Whisper Diarization @radford_robust_2023 to isolate utterances.

KIDS is a subset of the My Science Tutor Corpus @pradhan_my_2024 and contains children’s conversations with a virtual tutor in an educational setting. This tests if evaluation metrics can generalize to data rarely encountered during the training.
...
#strong[Listening Test: collecting human judgements across systems and datasets]
...
=== TTSDS benchmark: Multilingual & recurring evaluation
...
As our benchmark should be updated frequently to avoid data leakage, and represent a wide range of recording conditions, speakers and environments, we decide to automate the creation of the WILD dataset described earlier. However, since manual filtering is not feasible in the long term for a growing set of languages and evaluation re-runs, we automate the collection process as can be seen in the above figure, and which we describe in detail in the following section.

#comic.comic((80mm, 40mm), "Comic overview of TTSDS pipeline: Sources include YouTube in 14 Languages, LibriVox, My Science Tutor. TTS systems like E2, FishSpeech, TorToiSe, MaskGCT. Processes: Scrape Data, Preprocess, Filter, Synthesise, TTSDS. Outputs: Robust Correlations, Automatic Rerun, Public Ranking Objective Metric, Datasets, Listening Tests", blue) <fig_ttsds_pipeline>
