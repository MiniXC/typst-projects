...
#minorchange[Wording][#strike["significant"]][- #smallcaps[Kids]: To evaluate generalisation to a different speaker population, this dataset was created from a subset of the My Science Tutor Corpus @pradhan_my_2024, which contains conversational speech from children interacting with a virtual tutor.]
...
To contextualise the performance of TTSDS2, we compared it against 16 other publicly available objective metrics using the VERSA evaluation toolkit @shi_versa_2024. This comparison set included signal-based metrics (#abbr.l[STOI], #abbr.l[PESQ], #abbr.l[MCD]), several model-based #abbr.a[MOS] predictors (UTMOS, UTMOSv2, NISQA-MOS, DNSMOS, SQUIM-MOS), speaker similarity metrics based on different embeddings (X-Vector, RawNet3, ECAPA-TDNN), distributional metrics (#abbr.l[FAD]), and multi-dimensional perceptual metrics (Audiobox Aesthetics sub-scores). #majorchange[Addition][explain AE, add backreference][These sub-scores consist of Content Enjoyment (AE-CE), Content Usefulness (AE-CU), and Production Quality (AE-PQ) @tjandra_meta_2025. This comprehensive set allows for a thorough analysis of the current state of objective evaluation for high-quality synthetic speech. As in @08_exp, 100 reference utterances were used.]
...
#minorchange[Wording][#strike["significant"]][The next best performing category of metrics is speaker similarity, with RawNet3 and X-Vector achieving average correlations of 0.6. Among the #abbr.a[MOS] prediction networks, SQUIM-MOS is the strongest performer with an average correlation of 0.57. However, many other metrics, including several recent #abbr.a[MOS] predictors and the Audiobox Aesthetics sub-scores, show a performance drop when moving from in-domain clean and noisy audiobook data to the more challenging #smallcaps[Wild] and #smallcaps[Kids] domains. This highlights a common issue of overfitting or lack of generalisation in current learned objective metrics.]
