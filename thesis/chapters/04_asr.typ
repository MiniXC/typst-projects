...
#comic.comic((80mm, 40mm), "Comic depiction of hybrid HMM-DNN: DNN processing features into HMM states", green) <fig_hybrid_hmm_dnn>
...
#comic.comic((80mm, 40mm), "Comic illustration of CTC: blank tokens aligning acoustic frames to labels", yellow) <fig_ctc_model>

=== Sequence-to-sequence models

An alternative #abbr.a[E2E] paradigm is the sequence-to-sequence (seq2seq) model, often employing an attention mechanism @chan_listen_2016, as detailed in @prabhavalkar_survey_2023. These models consist of an encoder network, which maps the input acoustic sequence $S$ into a high-level representation, and a decoder network, which autoregressively generates the output transcription $T$ one token at a time. The core mechanism linking the two is attention, which at each decoding step computes a context vector as a weighted average of the encoder's output states. This allows the decoder to dynamically focus on the most relevant acoustic frames when predicting the next output token.
...
#comic.comic((80mm, 40mm), "Comic of seq2seq model: encoder-decoder with attention mechanism", purple) <fig_seq2seq>
...
Pretraining leverages large amounts of unlabeled audio data to learn robust acoustic representations, addressing data scarcity in supervised #abbr.a[ASR]. Popular models include wav2vec 2.0 @baevski_wav2vec_2020 and HuBERT @hsu_hubert_2021, which use contrastive or masked prediction tasks to capture phonetic and contextual information. During fine-tuning, a lightweight head (e.g., linear layer with CTC loss) is added to adapt the pretrained encoder for transcription. This approach yields state-of-the-art results on benchmarks like LibriSpeech, often outperforming models trained from scratch by 10-20% relative WER reduction, especially in low-resource scenarios (@prabhavalkar_survey_2023). Benefits include improved generalization to noisy or accented speech, and efficiency in transfer learning across languages or domains.

#comic.comic((80mm, 40mm), "Comic showing pretraining on masked audio followed by fine-tuning with CTC head", orange) <fig_pretrain_finetune>