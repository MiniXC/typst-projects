...
$ asrT = argmax_(T in cal(T)) P_theta (T|S) $

#majorchange[Addition][forward reference and clarification of ASR use][
The remainder of this thesis uses ASR as a diagnostic tool and proxy for the acoustic properties of synthetic speech. In @05_setup we define a controlled TTS-for-ASR procedure, and in @05_werr we analyse the resulting #abbr.a[WER] and #abbr.a[WERR] outcomes when training on real versus synthetic speech. To interpret these results, the key ASR background is (i) the role of speech variability in making ASR a many-to-one problem, (ii) the hybrid HMM-DNN paradigm and its sequence-discriminative training objectives, since our later baselines use a hybrid HMM-TDNN system trained with #abbr.a[LF-MMI] (see @05_hybrid), and (iii) the role of language modelling, as the subsequent chapters aim to keep lexical confounds fixed.]

Modern high-performance #abbr.a[ASR] systems are almost exclusively trained using discriminative objectives. In contrast to generative approaches that might model the joint probability $P(S, T)$, discriminative models are optimised to directly model the posterior probability $P(T|S)$, maximising the score of the correct transcription while simultaneously minimising the scores of all incorrect competing hypotheses. This approach has proven more effective at achieving low #abbr.a[WER]. The two dominant paradigms for discriminative #abbr.a[ASR] training are hybrid HMM-DNN systems and end-to-end models. For a comprehensive review of #abbr.a[E2E] #abbr.a[ASR] models, see #citep(<prabhavalkar_survey_2023>).
...
While TTS is a "one-to-many" problem where a single text can have many valid acoustic realisations, ASR is a "many-to-one" problem, as illustrated in @fig:fig_tts_asr_comp. A single intended text sequence can be realised as a vast number of acoustically distinct signals, all of which should map back to the same transcription. This variability is the central challenge of ASR and arises from the factors of speech discussed in @02_factors[Chapter]. Variability from the #smallcaps[Speaker] factor includes differences in vocal tract length, pitch, and accent. The #smallcaps[Prosody] factor introduces variability through changes in speaking rate, stress, and intonation. Finally, the #smallcaps[Ambient] factor adds variability through background noise, reverberation, and different recording channels.

To build a robust ASR system, this acoustic variability must be handled. Historically, this was primarily achieved through normalisation, a process designed to reduce "extrinsic" variability in the input signal before it reaches the acoustic model. To handle speaker variability, early systems used techniques like #abbr.a[VTLN] @eide_vtln_1996, which warps the frequency axis of the spectrum to compensate for differences in vocal tract size, effectively normalising low-level #smallcaps[Speaker] features like formant frequencies. #abbr.a[CMVN] @viikki_cmvn_1998 is another common technique that normalises feature vectors on a per-speaker or per-utterance basis to have zero mean and unit variance.


In contrast to this normalisation-based approach, the modern deep learning paradigm handles variability by making the model itself invariant through training on diverse data. This is often achieved with data augmentation, where the training set is artificially expanded by creating modified copies of the original audio. Common techniques include speed perturbation, which makes the model robust to changes in speaking rate (#smallcaps[Prosody]), and the addition of background noise and reverberation, which makes the model robust to different #smallcaps[Ambient] conditions, as well as masking part of the features such as with commonly used SpecAugment @park_specaugment_2019. By exposing the model to a wide range of acoustic variations during training, it should learn to focus more on the core linguistic content while ignoring the extrinsic factors, reducing potential overfitting.

=== Hybrid HMM-DNN Systems

Hybrid systems combine Deep Neural Networks (DNNs) with the temporal modeling structure of Hidden Markov Models (HMMs). This architecture has been a popular paradigm for high-performance ASR for many years and is centrally supported by toolkits like Kaldi @povey_kaldi_2011. In this setup, the DNN, often a #abbr.a[TDNN] @peddinti_time_2015, processes acoustic feature vectors $s_k$ at each timestep $k$ and outputs a posterior probability distribution over the set of HMM states $q$:

$ P(q_k|s_k; theta) $

While this acoustic model can be trained with a simple cross-entropy loss against frame-level alignments generated from a previous model, system performance is substantially improved through sequence-level discriminative training. #majorchange[Addition][forward reference][For hybrid ASR systems #abbr.a[LF-MMI] @povey_purely_2016 is a state-of-the-art objective and is used for the hybrid HMM-TDNN baselines in our controlled experiments in @05_hybrid.] The MMI criterion aims to maximise the mutual information between the observation sequence $S$ and the reference word sequence $T$. This is achieved by maximising the log-likelihood of the correct transcription (the numerator) while minimising the log-likelihood of all possible competing transcriptions (the denominator):

$ cal(L)_(text("MMI"))(theta) = log (P_theta (S | cal(M)_T) P(T)) / (sum_(T') P_theta (S | cal(M)_(T')) P(T')) $

Here, $cal(M)_(T')$ is the HMM decoding graph corresponding to a word sequence $T'$, and $P(T')$ is a language model probability. The "Lattice-Free" component streamlines this process by using a simpler phone-level decoding graph for the denominator, allowing for more efficient training. The conventional LF-MMI pipeline, however, still relies on a previously trained model to generate alignments for creating context-dependent decision trees. Addressing this, the work of @hadian_lfmmi_2018 introduces an "end-to-end" version of LF-MMI that operates in a flat-start manner. This approach eliminates the need for any previously trained models, forced alignments, or state-tying decision trees. It uses full biphones for context-dependency without tree-based tying. A key departure from the standard LF-MMI pipeline, necessitated by the lack of alignments, is that utterances are trained on whole, rather than being split into smaller chunks.
...
The probability of any single path is the product of the per-timestep softmax outputs from the network: 
$
P(bold(pi)|S) = product_(k=1)^K P(pi_k | S)
$
The CTC loss is the negative log-likelihood of this total probability:#footnote[Which can be calculated efficiently using dynamic programming.]
$
cal(L)_("CTC") = -log P(T|S)
$

By maximising the summed probability of all valid paths, the CTC objective implicitly pushes down the probability of all other label sequences, making it an inherently discriminative training criterion.

==== Sequence-to-Sequence Models

An alternative E2E paradigm uses sequence-to-sequence (seq2seq) models, often employing an attention mechanism @chan_listen_2016. These models consist of an encoder, which maps the input acoustic sequence $S$ into a high-level representation, and a decoder, which autoregressively generates the output transcription $T$ one token at a time. The core mechanism is attention, previously detailed in @03_arch, which at each decoding step computes a context vector as a weighted average of the encoder's outputs. This allows the decoder to dynamically "focus" on the most relevant acoustic frames when predicting the next output token. The model is trained to maximise the conditional probability of the target sequence, which is factorised using the chain rule:

$ P(T|S) = product_(k=1)^K P(t_k | t_(<k), S) $

The objective is a standard cross-entropy loss over the predicted token distributions: 

$
cal(L)_("att") = -sum_(i=1)^L log P(t_i | t_(<i), S)
$

While powerful, pure attention-based models do not explicitly enforce monotonic alignment, which can lead to instability. This has led to the development of hybrid CTC/attention models @watanabe_joint_2017 that combine both objectives in a multi-task learning framework. The CTC loss acts as a regulariser, encouraging monotonic alignments, while the attention component models global dependencies, resulting in more robust and accurate systems. The architectural differences between hybrid, CTC, and sequence-to-sequence models are summarised in @fig:fig_hybrid_encdec_ctc.
