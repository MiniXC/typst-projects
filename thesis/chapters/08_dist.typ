...
#majorchange[Clarification][explain 100 utterances][The reference speech datasets employed to compute the distributional distances in TTSDS include LibriTTS @zen_libritts_2019, LibriTTS-R @koizumi_libritts-r_2023, LJ Speech @ito_lj_2017, VCTK @vctk, and the training sets provided by the various Blizzard challenges @king_blizzard_2008@ling_blizzard_2021@le_maguer_back_2022. From each of these datasets, 100 utterances were randomly sampled, preferentially from their respective test splits if available. We selected 100 utterances as the reference set size to maintain computational tractability while ensuring the objective evaluation setup mirrors the scale of data typically presented to listeners in subjective evaluations. For the generation of distractor noise datasets, TTSDS utilised the ESC dataset @piczak_esc_2015 of background noises, in conjunction with synthetic noise types, including random uniform noise, random normal noise, and silent (all zeros and all ones) samples.]
