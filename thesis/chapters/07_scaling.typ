...
The #emph[forward diffusion process] is a fixed Markov chain that gradually adds Gaussian noise to a clean data sample ($x_0$, typically a melspectrogram in our case) over a series of $K$ timesteps. The process is defined by a variance schedule $beta_k in (0, 1)$ for $k=1, ..., K$:
...
#figure(
  ...
  placement: top,
  caption: [TTS model architecture utilising two U-Net models for prosody representation and melspectrogram generation.],
