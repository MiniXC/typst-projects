...
#minorchange[Wording]["Mel" #sym.arrow "mel"][The #emph[forward diffusion process] is a fixed Markov chain that gradually adds Gaussian noise to a clean data sample ($x_0$, typically a mel spectrogram in our case) over a series of $K$ timesteps.] The process is defined by a variance schedule $beta_k in (0, 1)$ for $k=1, ..., K$:
...
#majorchange[Addition][clarify why ASR dataset size is held constant][Holding the #abbr.a[ASR] training size constant removes a major confound and makes the results easier to interpret: changes in downstream #abbr.a[WER] can be attributed primarily to the #abbr.a[TTS] model's training scale and objective. We additionally report results across a wider range of #abbr.a[ASR] training set sizes in @tbl:tab_synthetic_asr_wer to show how the observed trends change when both #abbr.a[TTS] and #abbr.a[ASR] scale.]
