...
The fact that ASR data augmentation is frequently used shows a further limitation: Techniques like SpecAugment improve ASR but distort distributions away from real speech @park_specaugment_2019. Thus, MWERR is a task-specific heuristic for dissimilarity, not a true metric. The WER itself is the outcome of a complex, non-linear optimisation process during ASR model training and decoding, not a direct comparison of underlying data distributions. Therefore, while WERR gives us an empirical way to compare previous works and conclude that a sizeable gap exists, it cannot precisely quantify the specific causes of this gap. 
