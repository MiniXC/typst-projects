...
Further analysis reveals the effects of synthetic speech's inherent regularity. When the ASR model trained on real speech is evaluated on synthetic speech (i.e., on $tilde(D)^"ASR"_"test"$), its WER is surprisingly low at 11.4%, which is better than its performance on natural-speech evaluation data (13.3%). #majorchange[Clarification][lessen oversmoothing claim][This does not contradict the synthetic-real gap measured by WERR, as the latter is defined with respect to evaluation on real speech. Instead, it highlights that the synthetic test distribution may be simpler: it might be cleaner, more consistent, and exhibit an oversmoothed quality, as found in prior work @ren_revisiting_2022. An ASR model trained on real speech could therefore perform better for transcribing this simplified input.]
