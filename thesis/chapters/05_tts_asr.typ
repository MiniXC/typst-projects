...
While synthetic data is frequently used to augment real datasets, in this work, we primarily treat it as a proxy for real speech. Throughout this chapter, the term #emph[synthetic-real gap] refers to the difference in #abbr.a[WER] between an ASR system trained on real speech and an otherwise equivalent ASR system trained on TTS-generated speech, when both are evaluated on natural-speech test data.
...
WERR is intentionally directional: in the main use case of this thesis, we care about whether an ASR system trained on synthetic speech transfers to natural-speech evaluation. This makes $"WERR"(D,tilde(D))$ the primary quantity of interest.
...
Further analysis reveals the effects of synthetic speech's inherent regularity. When the ASR model trained on real speech is evaluated on synthetic speech (i.e., on $tilde(D)^"ASR"_"test"$), its WER is surprisingly low at 11.4%, which is better than its performance on natural-speech evaluation data (13.3%). This does not contradict the synthetic-real gap measured by WERR, because the latter is defined with respect to evaluation on real test audio. Instead, it highlights that the synthetic test distribution can be simpler: it tends to be cleaner, more consistent, and exhibits an oversmoothed quality @ren_revisiting_2022. An ASR model accustomed to the variability of real speech can therefore find this "idealised" input easier to transcribe.
...
While the choice of vocoder is critical for the perceptual quality of TTS output, its impact on the TTS-for-ASR utility gap appears to be minimal. This can be quantified by computing the WERR on re-synthesised data, where a vocoder representation is extracted from original audio and then used to synthesise a new waveform. An experiment by #citep(<rossenbach_duration_2023>) using the algorithmic Griffin-Lim vocoder resulted in a WERR of 1.04, suggesting the vocoder step accounts for at most 5% of the performance gap. Our own preliminary experiments using a modern, high-fidelity neural vocoder, Hifi-GAN @kong_hifigan_2020, yielded a WERR close to 1.00, as can be seen in @tbl:tab_cross_ttsasr. Thus only minor differences in WER between ASR models trained on original real data and those trained on data that was converted to mel spectrograms and then back to waveforms are present. This result is not unexpected, as mel spectrograms are a common input representation for ASR systems themselves @jurafsky_slp_2008. This implies that the lossy-reconstructible transformation to a mel spectrogram preserves most of the information relevant to ASR, and the final vocoding step, if performed with sufficient fidelity, does not cause the majority of the synthetic-real gap. The primary source of the gap must therefore lie in the acoustic model that generates the spectrograms, not the vocoder that inverts them. This chapter has established and quantified the synthetic-real gap in ASR training using the WERR heuristic. We have demonstrated through a review of prior work and our own baseline experiments that a large performance disparity persists, and we have ruled out the vocoder as a primary cause. The evidence points to a more fundamental issue: the acoustic models of current TTS systems fail to capture the full distributional diversity of real speech.

While the choice of vocoder is critical for the perceptual quality of TTS output, its impact on the TTS-for-ASR utility gap appears to be minimal. This can be quantified by computing the WERR on re-synthesised data, where a vocoder representation is extracted from original audio and then used to synthesise a new waveform. An experiment by #citep(<rossenbach_duration_2023>) using the algorithmic Griffin-Lim vocoder resulted in a WERR of 1.04, suggesting the vocoder step accounts for at most 5% of the performance gap. Our own preliminary experiments using a modern, high-fidelity neural vocoder, Hifi-GAN @kong_hifigan_2020, yielded a WERR close to 1.00, as can be seen in @tbl:tab_cross_ttsasr.

Only minor differences in WER between ASR models trained on original real data and those trained on data that was converted to mel spectrograms and then back to waveforms are present. This result is not unexpected, as mel spectrograms are a common input representation for ASR systems themselves @jurafsky_slp_2008. In our setup, this suggests that (i) the mel-spectrogram representation retains most information relevant to recognition, and (ii) vocoding, if performed with sufficient fidelity, is unlikely to explain most of the synthetic-real gap. The remaining discrepancy therefore points more strongly to limitations introduced earlier in the TTS pipeline, such as the acoustic model that generates the spectrograms.

This chapter has established and quantified the synthetic-real gap in ASR training using the WERR heuristic. We have demonstrated through a review of prior work and our own baseline experiments that a large performance disparity persists, and we have found no evidence that the vocoder is a dominant contributor in the controlled setting considered here. The evidence instead points to a more fundamental issue: current TTS acoustic models fail to capture the full distributional diversity of real speech.
