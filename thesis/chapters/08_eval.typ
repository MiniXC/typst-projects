...
== Synthetic speech evaluation and data collection <08_eval>
...
A general best practice is clearly answer the following questions before conducting a test, as recommended in guidelines for #abbr.a[TTS] evaluation @wester_listeners_2015 @cooper_review_2024:
#enum(
[
Who are the listeners? Usually, native speakers of the language of the samples are preferred. Additionally, certain tests require a significant number of different speakers to be used @wester_listeners_2015, to mitigate biases from individual perceptual differences or fatigue effects. For example, in large-scale subjective evaluations, annotators are often screened to be native speakers from specific regions and are required to use headphones in a quiet environment, with attention checks embedded in the survey to exclude unreliable responses.
],
[
What is the setting? E.g. #sym.arrow In a lab or online? If possible, requiring use of headphones in a quiet environment is preferred, to minimize external noise interference and ensure consistent audio playback @huang_voicemos_2024. For online crowd-sourced studies, clear instructions on environmental conditions are provided, although direct control over these external factors is inherently limited.
],
[
What are the instructions? Recently, #emph[naturalness] is the most commonly evaluated attribute of synthetic speech, and this should be phrased in an understandable way in the instructions, such as specifying that it includes prosody, intelligibility, and absence of artifacts. This ensures listeners evaluate the intended dimensions of speech quality.
],
[
What is the data? E.g. #sym.arrow Is the reference or an anchor @schoeffler_mushra_2015 included in the set of samples to be evaluated? Anchors, like degraded audio samples, can calibrate listener expectations and improve rating consistency @cooper_review_2024. The inclusion of ground truth samples, which are actual human recordings, allows for a direct comparison of synthetic speech against real speech.
],
[
What are the concrete stimuli? E.g. #sym.arrow How many samples are presented at one time, can listeners re-listen to the same sample? How many points are on the evaluation scale, and are they labeled? Allowing re-listening reduces memory bias, and labeled scales enhance reliability @wells_bws_2024. The order of different test parts, such as #abbr.a[MOS] and #abbr.a[CMOS] sections, is typically randomized for each annotator to mitigate learning or fatigue effects.
]
)
...
In practice, #abbr.a[MOS] involves presenting listeners with isolated audio samples and asking them to rate on the 5-point scale, often with intermediate labels like "Poor" (2), "Fair" (3), and "Good" (4) @ITU_P800. Scores are then averaged across listeners to yield a system-level #abbr.a[MOS]. This method is widely used in #abbr.a[TTS] challenges, such as the Blizzard Challenge @king_blizzard_2008, where it has been employed to compare systems over time. However, criticisms include its susceptibility to ceiling effects in high-quality #abbr.a[TTS] systems, where scores cluster near 5, reducing discriminative power. Moreover, #abbr.a[MOS] ratings can shift over time as listener expectations evolve; for instance, systems rated highly in 2008 may score lower today due to advancements in #abbr.a[TTS] @le_maguer_back_2022. Despite these issues, #abbr.a[MOS] remains a benchmark for validating objective metrics. For a survey, listeners are typically presented with a set number of audio recordings, such as 6 sets each containing 5 samples of the same text. One of these samples is always a ground-truth recording, while the others are randomly selected synthetic samples. The verbatim instructions given to listeners can be seen in @fig_mos_instructions.
...
#abbr.l[PESQ] estimates perceived quality by modeling auditory perception, accounting for distortions like delay and filtering @rix_pesq_2001. #abbr.l[STOI] focuses on intelligibility by correlating short-time spectral envelopes @taal_stoi_2011. #abbr.l[MCD] measures spectral differences using Mel-cepstral coefficients, often used in #abbr.a[TTS] to quantify timbre mismatch @kominek_mcd_2008. These metrics are intrusive, requiring time-aligned references, which limits their use in zero-shot #abbr.a[TTS]. While effective for early systems, they struggle with modern #abbr.a[TTS] artifacts like prosodic inconsistencies @cooper_review_2024.

*Model-based:*
To predict scores directly, researchers train neural networks that map a single audio signal to an estimated #abbr.a[MOS]. #emph[MOSNet] @lo_mosnet_2019 introduced the idea, and was followed by #emph[UTMOS] @saeki_utmos_2022, its #abbr.a[SSL]‑based successor #emph[UTMOSv2] @baba_t05_2024, and #emph[NISQA‑MOS] @mittag_nisqa_2021. #emph[SQUIM-MOS] @kumar_torchaudio-squim_2023 additionally grounds its prediction by requiring a non-matching reference of the ground truth speech. These methods report in‑domain correlations; however, recent VoiceMOS challenges @huang_voicemos_2024 show that correlation with subjective ratings drops out-of-domain. The drawbacks listed in @08_distances make training these networks challenging, and generalisation to new #abbr.a[TTS] systems or domains unlikely -- as the quote from @moller_quality_2009 at the beginning of this chapter indicates, each time substantially different #abbr.a[TTS] systems are introduced, learned metrics will have to be adjusted and verified anew. A recent prediction system goes beyond #abbr.a[MOS] and argues that no single score can capture everything listeners care about. #emph[Audiobox Aesthetics] predicts four axes, Production Quality, Complexity, Enjoyment, and Usefulness, for arbitrary audio @tjandra_meta_2025 -- however, the challenges of learning from subjective labels remain the same, including domain shifts and the need for large, diverse training sets of rated audio.

*Intelligibility:*
Often reported are also #abbr.l[WER] and #abbr.l[CER], computed on #abbr.a[ASR] transcripts. These metrics quantify how easily synthetic speech can be transcribed by an #abbr.a[ASR] system, serving as a proxy for human intelligibility @cooper_review_2024. Lower #abbr.l[WER] indicates clearer pronunciation, but in domains like children's speech, higher #abbr.l[WER] may be realistic. They are non-intrusive but require #abbr.a[ASR] models, and performance depends on the #abbr.a[ASR]'s robustness @radford_robust_2023.

*Speaker similarity:*
Analogous to #abbr.a[SMOS], cosine similarity between the speaker embeddings (see @02_speaker) of a reference and target speech is frequently reported. Systems like d-vectors @wan_generalized_2018 or ECAPA-TDNN @desplanques_ecapa_2020 compute embeddings, and similarity is the cosine of their angle @zhao_probing_2022. This non-intrusive metric correlates with #abbr.a[SMOS] but may overlook prosodic influences on perceived similarity.

*Distributional:* Inspired by the image domain's #abbr.l[FID] @heusel_fid_2017, audio researchers proposed measuring entire corpora rather than single files. #abbr.l[FAD] @kilgour_fad_2019 compares embeddings and has since been adapted for #abbr.a[TTS] @shi_versa_2024. Distributional metrics require a set of references which do not need to correspond to the synthetic data. The authors of these metrics state the need for thousands of samples, which may be why they have not found more widespread adoption. We go into more detail into the workings of these metrics in the next Chapter, where we propose #abbr.l[TTSDS] as a factorized distributional metric that correlates robustly with subjective scores across domains and languages.

=== Data Collection and Processing

Here, we detail the methodology for collecting and processing data for subjective listening tests, with particular attention to ethical considerations and the practical pipeline steps involved in generating diverse and representative synthetic speech. The objective is to ensure that the collected data is suitable for robust evaluation of #abbr.a[TTS] systems, addressing both human perception and the practical implications of synthetic speech generation.

==== Ethical Considerations

#figure(
  image("../figures/8/real_fake_tts.png", width: 100%),
  placement: top,
  caption: [TTS evaluation as resynthesis of existing utterances.],
) <fig_ethical_tts>

When collecting and generating speech data, particularly for voice cloning scenarios, ethical considerations are critical. Our methodology adheres to strict ethical guidelines to ensure speaker privacy and prevent the creation of synthetic speech that misrepresents an individual. We obtain ethical approval for our studies through the Informatics Research Ethics Process, providing participants with comprehensive information sheets that detail the purpose and terms of the study. Participants' consent is collected explicitly, and they are given a period during which they can withdraw their consent.

A key ethical principle guiding our data collection is the explicit avoidance of generating novel speech content that a speaker did not utter in the original recording. To achieve this, we employ a specific two-utterance per speaker approach for voice cloning. As shown in @fig_ethical_tts, when a #abbr.a[TTS] system is used to clone a speaker's voice, one of these utterances serves as the "reference" for voice characteristics, while the text content of the second, entirely distinct utterance is used as the input for synthesis. This ensures that the generated synthetic speech, while mimicking the speaker's voice, only articulates content that the speaker themselves has previously spoken. This methodology allows for the evaluation of TTS through resynthesis, without inadvertently creating or circulating speech that could be falsely attributed to the original speaker. This approach mitigates risks such as identity theft or misinformation.

==== Listening Test Logistics

In our work, listening tests conducted using online platforms specialized for crowd-sourcing -- specifically, Qualtrics for creating and running surveys and Prolific for recruitment of a large and diverse pool of annotators.

For each evaluation round, a substantial number of annotators are recruited -- we surpass the number of 30 annotators recommended by #citep(<wester_listeners_2015>), opting for 50. Annotators undergo a screening process, which includes verification of native language proficiency and a requirement to complete the tasks in a quiet environment using headphones. Embedded attention checks throughout the survey are crucial for identifying and excluding annotators who may not be paying adequate attention or are providing inconsistent responses, thereby preserving data quality -- for our tests, one attention check was included for every 10 minutes of expected completion time.
Participants are compensated for their time at a rate that exceeds standard hourly wages for similar tasks, ensuring fair remuneration for their contributions. The median completion time for a full survey is tracked to optimize task design and compensation, and we aim for a completion time of 30 minutes to avoid fatigue. We also avoid presenting more than 6 stimuli per survey page for the same reason. The survey itself is structured into multiple parts, covering different evaluation criteria such as Mean Opinion Score (#abbr.a[MOS]), Comparison #abbr.a[MOS] (#abbr.a[CMOS]), and Speaker Similarity #abbr.a[MOS] (#abbr.a[SMOS]). The order of these sections is randomized for each individual annotator to mitigate potential learning or fatigue effects over the course of the test.
Detailed instructions for each type of listening test are provided to the annotators to ensure clarity and consistency in their ratings. These instructions specify the rating scales, the criteria for evaluation (e.g., "naturalness" or "similarity"), and examples of how to interpret the scale points. As an example, the precise wording of instructions for the #abbr.a[MOS] task, which asks listeners to rate naturalness on a 1-5 scale, can be seen in @fig_mos_instructions. Additionally, an example survey can be accessed at #link(
  "https://ttsdsbenchmark.com/survey",
  underline[ttsdsbenchmark.com/survey],
). These detailed instructions, coupled with attention checks and strict participant screening, aim to produce high-quality human judgments that accurately reflect the perceived quality of synthetic speech.

In the next chapter we show how we verify an objective distributional benchmark using the listening test techniques explained here.


#figure(
  image("../figures/8/mos_instructions.png", width: 75%),
  placement: top,
  caption: [Example MOS Instructions for listening tests.],
